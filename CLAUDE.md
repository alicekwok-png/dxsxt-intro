...
├── build.py                  ← 建置腳本：讀 src/ + assets/，輸出做一個 self-contained 嘅 dist/index.html
├── dist/
...
4. **唔可以出現未經證實嘅回報數字承諾**（例如帶暗示性嘅「~500%回報」）。代幣解鎖時間
   ...
   格式。
5. **中英文措辭一律保持對齊，改一邊就要即刻改另一邊**（用戶明確要求，唔使再問）。例如
   風險聲明而家中文淨係講「風險」、英文都對應淨係講 "risk"，冇加強度形容詞如
   「重大/高度/significant」；hero 中文淡化咗「高利潤」之後，英文大標題亦都由
   "MAXIMIZED RETURNS." 改成 "REAL YIELD."。改任何一邊文案之前，搜索返對應嗰邊有冇
   同樣意思嘅句子要一齊跟。
6. **網頁正文一律用簡體中文 + 普通話書面語**（唔好用廣東話口語詞：嘅/喺/咗/佢/唔/冇/
